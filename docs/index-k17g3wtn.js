...
function renderEditor() {
  ...
  const saveGradientBtnMobile = document.getElementById("save-gradient-btn-mobile");
  if (!state.activeSegmentId) {
    if (panel)
      panel.classList.remove("has-active-segment");
    if (controls)
    ...
    if (saveGradientBtnMobile)
      saveGradientBtnMobile.disabled = true;
    return;
  }
  ...
  if (!segment)
    return;
  if (panel)
    panel.classList.add("has-active-segment");
  if (controls)
  ...
  if (saveGradientBtnMobile)
    saveGradientBtnMobile.disabled = false;
...
function renderGradientsToContainer(containerId) {
